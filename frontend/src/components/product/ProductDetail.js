...
export default function ProductDetail(){
     ...
     const {id}=useParams() //getting parameter (id)
...
<button disabled={loading} onClick={reviewHandler} aria-label="Close"
    className='btn my-3 float-right review-btn px-4 text-white'>Submit
</button>
        </Modal.Body>
        {/* <Modal.Footer>
...
</div> {product.reviews && product.reviews.length>0 ? <ProductReview reviews={product.reviews} /> :null
}
            </Fragment>}
